...
#--------------------------------------------------------------------------
# qprof version 0.4e - Oct 2022 - maurizio.felici@vertica.com
...
#---------------------------------------------------------------------------
QPV="qprof version 0.4e - Oct 2022"
OUT=qprof.out
CLC=0
GZP=0
RPOOL=""
...
if [ ! -z ${SQL+x} ] ; then
    ...
    trxst=$(sed "1s/^/${RPOOL} profile /" ${SQL} | 
            ...
            sed -n '/^HINT:.*$/H;${x;s/.*=\([0-9]*\).*=\([0-9]*\);$/\1,\2/p}')
    if [ -z "${trxst}" ] ; then
...
cat <<-EOF | vsql -X -q -P null='(null)' -o ${OUT} -f -
    ...
    -- ------------------------------------------------------------------------
    \echo '    Step 11: Threads by node and path_id'
    \qecho >>> Step 11: Threads by node and path_id
    SELECT
        node_name, 
        path_id,
        operator_name, 
