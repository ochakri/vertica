...
cat <<-EOF | vsql -Xqn -P null='NULL' -o ${OUT} -f -
 ...
 -- ;
  ...
  \qecho >>> Step 15h: Additional Testing Queries - Connections Initiated(VAdvisor format)
	...
	select /*+label(isLoadBalanced)*/ right(node_name,3)::int as node_num
	...
	from ${VINT}.dc_requests_issued a join ${VINT}.vs_nodes b on a.node_name = b.name and nodetype = 0 group by 1 order by node_num;
