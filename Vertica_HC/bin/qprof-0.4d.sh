...
#---------------------------------------------------------------------------
QPV="qprof version 0.4d - May 2020"
OUT=qprof.out
CLC=0
GZP=0
RPOOL=""
...
usage+=" -gz to compress the output file\n"
...
cat <<-EOF | vsql -X -q -P null='(null)' -o ${OUT} -f -
    ...
    -- Vertica Version
    -- ------------------------------------------------------------------------
    \echo '    Step 0: Vertica version'
    \qecho >>> Step 0: Vertica version
    SELECT
        VERSION() 
    ;
    ...
    -- Query text
    -- ------------------------------------------------------------------------
    \echo '    Step 1: Query text'
    \qecho >>> Step 1: Query text
    \pset tuples_only
    SELECT
        request 
    FROM 
    ...
    -- Query duration
    -- ------------------------------------------------------------------------
    \echo '    Step 2: Query duration'
    \qecho >>> Step 2: Query duration
    SELECT
        query_duration_us 
    FROM 
    ...
    -- ------------------------------------------------------------------------
    \echo '    Step 3: Query execution steps'
    \qecho >>> Step 3: Query execution steps
    SELECT
        execution_step, 
    ...
    -- ------------------------------------------------------------------------
    \echo '    Step 4: Resource Acquisition'
    \qecho >>> Step 4: Resource Acquisition
    SELECT
        a.node_name,
    ...
    -- Query plan
    -- ------------------------------------------------------------------------
    \echo '    Step 5a: Query execution plan'
    \qecho >>> Step 5a: Query execution plan
    SELECT
    ...
    -- Query plan
    -- ------------------------------------------------------------------------
    \echo '    Step 5b: Query plan profile'
    \qecho >>> Step 5b: Query plan profile
    SELECT
        statement_id AS stmtid,
        path_id,
        running_time,
        memory_allocated_bytes//(1024*1024) AS mem_mb,
        read_from_disk_bytes//(1024*1024) AS read_mb,
        received_bytes//(1024*1024) AS in_mb,
        sent_bytes//(1024*1024) AS out_mb,
        left(path_line, 70) as path_line
    ...
    -- ------------------------------------------------------------------------
    \pset expanded
    \echo '    Step 5c: Query consumption'
    \qecho >>> Step 5c: Query consumption
    SELECT
        *
    ...
    -- ------------------------------------------------------------------------
    \echo '    Step 6: Elapsed & memory allocated by node, path_id and activity'
    \qecho >>> Step 6: Elapsed & memory allocated by node, path_id and activity
    SELECT
        a.node_name, 
        a.path_id, 
        ...
        b.path_line,
        a.description, 
        ROUND(a.assigned_memory_bytes/1000000, 2.0) AS mem_mb 
    FROM 
    ...
    -- ------------------------------------------------------------------------
    \echo '    Step 7: Elapsed, exec_time and I/O by node, activity & path_id'
    \qecho >>> Step 7: Elapsed, exec_time and I/O by node, activity & path_id
    SELECT
        node_name,
        path_id,
        activity,
    ...
    -- ------------------------------------------------------------------------
    \echo '    Step 9: Suggested Action Summary'
    \qecho >>> Step 9: Suggested Action Summary
    SELECT
        path_id, 
    ...
    -- ------------------------------------------------------------------------
    \echo '    Step 10: CPU Time by node and path_id'
    \qecho >>> Step 10: CPU Time by node and path_id
    SELECT
        node_name, 
        path_id, 
        cet, 
    ...
    -- ------------------------------------------------------------------------
    -- Threads per operator by node, path_id and "abl"
    -- ------------------------------------------------------------------------
    \echo '    Step 11: Threads by node, path_id and abl_id'
    \qecho >>> Step 11: Threads by node, path_id and abl_id
    SELECT
        node_name, 
        path_id,
        operator_name, 
    ...
    -- ------------------------------------------------------------------------
    \echo '    Step 12: Query execution report'
    \qecho >>> Step 12: Query execution report
    SELECT
            node_name ,
            operator_name,
            path_id,
    ...
    -- ------------------------------------------------------------------------
    -- Transaction locks (contributed by Ilia Gulman)
    -- ------------------------------------------------------------------------
    \echo '    Step 13: Transaction locks'
    \qecho >>> Step 13: Transaction locks
    SELECT
        node_name,
    ...
    -- ------------------------------------------------------------------------
    \echo '    Step 14: Projection Data Distribution'
    \qecho >>> Step 14: Projection Data Distribution
    SELECT
        ...
        SUM(s.wos_row_count) AS wos_rows,
        SUM(s.wos_used_bytes) AS wos_bytes, 
        SUM(s.ros_row_count) AS ros_rows,
        SUM(s.ros_used_bytes) AS ros_bytes,
    ...
    SELECT
        DISTINCT 
            ...
            s.wos_row_count AS wos_rows,
            s.wos_used_bytes AS wos_bytes, 
            s.ros_row_count AS ros_rows,
            s.ros_used_bytes AS ros_bytes,
            s.ros_count AS ros_count,
            sc.deleted_row_count AS del_rows,
            sc.delete_vector_count AS DVC
    FROM
    ...
    -- ------------------------------------------------------------------------
    \echo '    Step 15: Query execution profile counters extraction in CSV format'
    \qecho >>> Step 15: Query execution profile counters extraction in CSV format
    ...
    -- ------------------------------------------------------------------------
    \pset expanded
    \echo '    Step 16: Getting Vertica non-default configuration parameters'
    \qecho >>> Step 16: Getting Vertica non-default configuration parameters
    SELECT
        parameter_name, 
        current_value, 
        default_value, 
    ...
    -- ------------------------------------------------------------------------
    \pset expanded
    \echo '    Step 17: Getting RP configuration'
    \qecho >>> Step 17: Getting RP configuration
    SELECT
        *
    FROM
    ...
    -- ------------------------------------------------------------------------
    \pset expanded
    \echo '    Step 18: Getting Cluster configuration'
    \qecho >>> Step 18: Getting Cluster configuration
    SELECT
        *
    FROM
    ...
    -- ------------------------------------------------------------------------
    \echo '    Step 19: Getting Projection Definition'
    \qecho >>> Step 19: Getting Projection Definition
EOF

while read p; do
    vsql -X -t -q -c "SELECT export_objects('','${p}')"
